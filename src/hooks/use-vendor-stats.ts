  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [statsResult, profileResult] = await Promise.all([
        supabase.from("vendor_stats").select("*").eq("vendor_id", vendorId).maybeSingle(),
        supabase.from("profiles").select("first_name, last_name, company_name, average_rating, total_reviews, events_completed").eq("id", vendorId).single(),
      ]);
