...
interface FileAttachmentProps {
  ...
  folderPrefix: string; // e.g. userId or weddingId
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (bucket === 'team-files') {
        ...
        onFileUploaded(signedData?.signedUrl || publicUrl, file.name);
      } else {
...
// Preview component for file attachments in message bubbles
export const FilePreview = ({ fileUrl, fileName, className }: { fileUrl: string; fileName: string; className?: string }) => {
    ...
    return (
      <a href={fileUrl} target="_blank" rel="noopener noreferrer" className={cn('block mt-1', className)}>
        <img
          src={fileUrl}
          alt={fileName}
          className="max-w-[200px] max-h-[150px] rounded object-cover border"
        />
  ...
  return (
    <a
      ...
      className={cn('flex items-center gap-2 mt-1 p-2 rounded border bg-background/50 hover:bg-background/80 transition-colors', className)}
    >
